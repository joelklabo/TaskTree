...
Notes:
- `make lint-backend` runs Ruff with `--fix` before mypy/bandit/yamllint.
...
- Frontend package manager: `pnpm` (via Corepack). Frontend lint/typecheck logs are tee’d into `logs/npm/*.log`.
