...
- [x] Scenario 2: Flow graph renders — Capture flow detail graph for `log_error_handler`; assert graph renders via Peekaboo artifact. (commit: a26204c)
- [x] Scenario 3: Run start feedback — After traced run start, capture toast + “No run selected” pill update. (commit: a26204c)
