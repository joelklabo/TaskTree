...
- [x] **Baseline**: Inspect current `copilot_cli` flow, note execution touch points, and choose minimal guardrails to port (timeouts, allow/deny, safe rm/find) without Bun deps.
...
- [x] **Refactor copilot_cli**: Swap to the new base/runner/validator, keep existing prompts/config; add config toggles (execute_commands, allowlist/denylist, timeout_sec, dry_run, backend choice).
...
### Baseline notes
- `copilot_cli` executes suggested commands in `_maybe_run_commands` via `subprocess.run(shell=True)` without timeouts or safety checks; dry-run is the only guard.
