...
if ! "$ROOT/scripts/refresh_tmux_dashboard.sh" --session "$SESSION" >/tmp/ttx-e2e-refresh.log 2>&1; then
...
if ! "$ROOT/scripts/share_pane.sh" "$SESSION" "dashboard" "$LOG_PANE_INDEX" "$LOG_PANE_TITLE" "$LOG_PANE_CMD" "$LOG_PANE_PID" "$LOG_PANE_PATH" >/tmp/ttx-e2e-share.log 2>&1; then
